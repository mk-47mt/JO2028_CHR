...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        // Trouver l'ID de l'athlète et de l'épreuve
        $statementAthleteID = $connexion->prepare("SELECT id_athlete FROM ATHLETE WHERE nom_athlete = :nomAthlete AND prenom_athlete = :prenomAthlete");
        $statementAthleteID->bindParam(":nomAthlete", $nomAthlete, PDO::PARAM_STR);
        $statementAthleteID->bindParam(":prenomAthlete", $prenomAthlete, PDO::PARAM_STR);
        $statementAthleteID->execute();
        $athlete = $statementAthleteID->fetch(PDO::FETCH_ASSOC);
        ...
        $idAthlete = $athlete['id_athlete'];

        // Trouver l'ID de l'épreuve
        $statementEpreuveID = $connexion->prepare("SELECT id_epreuve FROM EPREUVE WHERE nom_epreuve = :nomEpreuve");
        $statementEpreuveID->bindParam(":nomEpreuve", $nomEpreuve, PDO::PARAM_STR);
        $statementEpreuveID->execute();
        $epreuve = $statementEpreuveID->fetch(PDO::FETCH_ASSOC);

        if (!$epreuve) {
            $_SESSION['error'] = "Épreuve introuvable.";
            header("Location: add-results.php");
            exit();
        }

        $idEpreuve = $epreuve['id_epreuve'];

        // Requête pour ajouter un résultat dans la table PARTICIPER
        $queryResultat = "INSERT INTO PARTICIPER (id_athlete, id_epreuve, resultat) 
                          VALUES (:idAthlete, :idEpreuve, :resultat)";
        $statementResultat = $connexion->prepare($queryResultat);
        $statementResultat->bindParam(":idAthlete", $idAthlete, PDO::PARAM_INT);
        $statementResultat->bindParam(":idEpreuve", $idEpreuve, PDO::PARAM_INT);
